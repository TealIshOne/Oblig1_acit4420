    ...
    def numValidRead(self):
        ...
        try:
            ...
            if valid_readings:
                ...
                self.Sortedheart_rate=None
                self.Sortedskin_response=None
                self.Sortedtemperature=None
                self.Sortedactivity_level=None

                return "the containers are empty, there are no valid readings"
    ...
    def summary_sr(self):
        ...
        try:
            if self.Sortedskin_response is None:
                return None #, "missing data set"
            elif (isinstance(self.Sortedskin_response, list)):
                    if not self.Sortedskin_response:
        ...
        try:
            if self.Sortedactivity_level is None:
                return "missing data set"
                    ...
                    if not self.Sortedactivity_level:
    ...
    def recovery_tracker(self):
        ...
        try: 
            if len(middle)> 0 and len(end) > 0:
                middle_hrs = [x["heart_rate"] for x in middle if isinstance(x, dict) and x.get("heart_rate") is not None]
                end_hrs = [y["heart_rate"] for y in end if isinstance(y, dict) and y.get("heart_rate") is not None]

                avg_middle=sum(middle_hrs)/len(middle)
                avg_end=sum(end_hrs)/len(end)
                ...
                return "you are missing vital data to complete recovery check"
        except:
            return "something went wrong recovery_check function (line 453)"
